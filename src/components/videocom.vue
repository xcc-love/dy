<template>
  ...
  <van-action-sheet v-model:show="chatShow" :overlay="false" title="评论">
    <div class="chatContent" v-for="(item, i) in chatArr" :key="i">
      ...
      <div>
        <van-icon
          @click="handleChatLike(item)"
          :name="item.chatLikeShow ? 'like' : 'like-o'"
          size="16"
          :color="item.chatLikeShow ? '#f9476f' : ''"
        />
        &nbsp;&nbsp;
        <van-icon
          @click="item.chatTextShow = !item.chatTextShow"
          :name="item.chatTextShow ? 'eye' : 'closed-eye'"
          size="16"
        />
      </div>
    </div>
...
let chatCount = computed(() => {
  return videoStore.playsData.videos[chatIndex.value]?.chat.length ?? 0;
});
